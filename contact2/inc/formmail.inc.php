...
//------------------------------------------------------------------------------
  ...
  *****************************************************/
          ...
          if (isset($HTTP_POST_FILES) and !empty($HTTP_POST_FILES)) {
            ...
            if (isset($attachment) 
                ...
                while(list($field, $values) = each($_FILES))
                {
                    ...
                    while(list($ext_key, $ext_val) = each($allowed_files))
                    {
                        ...
                        if ($upload->saveAs($file_name . '_' . $upload_suffix, $path['upload'], $field, true)) {
                            
                            $attachment_file_names[] = array('new' => $path['upload'] . $file_name . '_' . $upload_suffix,
                                                             'old' => $file_name);
                ...
                if (isset($attachment)  and $attachment == 'yes') {
                    if ($file_suffix = gpc_vars('fs') and $file_list = gpc_vars('fn')) {
                        if (is_array($file_list) and sizeof($file_list) > 0) {
                            ...
                            {
                                if (!is_file($path['upload'] . $val . '_' . $file_suffix[$key])) {
                                    continue;
                                }
                                $attachment_file_names[] = array('new' => $path['upload'] . $val . '_' . $file_suffix[$key],
                                                                 'old' => $val);
