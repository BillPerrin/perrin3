...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://perr.in/cgi-bin/mt/mt-comments.cgi?__mode=userinfo&blog_id=2&jsonp=' + cb + '&ts=' + ts + '&sid=' + u.sid;
...
function mtVerifySession(cb) {
    ...
    var ts = new Date().getTime();
    var u = mtGetUser();
    script.src = 'http://perr.in/cgi-bin/mt/mt-comments.cgi?__mode=verify_session&blog_id=2&jsonp=' + cb + '&ts=' + ts + '&sid=' + u.sid;
    (document.getElementsByTagName('head'))[0].appendChild(script);
}
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = 'b60299e50f35e5c15811bcd05e4f6cb920a6266d';
...
function mtSignIn() {
    ...
    var url = 'http://perr.in/cgi-bin/mt/mt-cp.cgi?__mode=login&blog_id=2';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fperrin3.com%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://perr.in/cgi-bin/mt/mt-cp.cgi?__mode=logout&static=0&blog_id=2';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            ...
            var user_link;
            if ( u.is_author ) {
                user_link = '<a href="http://perr.in/cgi-bin/mt/mt-comments.cgi?__mode=edit_profile&blog_id=2&return_url=' + encodeURIComponent( location.href );
                user_link += '">' + u.name + '</a>';
            } else {
